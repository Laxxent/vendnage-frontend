...
import { useFetchStockIn, useUpdateStockIn } from "../../hooks/useStockManagement";
...
const EditStockIn = () => {
  ...
  const { data: stockIn, isPending } = useFetchStockIn(Number(id));
  ...
  useEffect(() => {
    // Wait for both stockIn data and products list to be loaded
    if (stockIn && !isPending && !productsLoading && products) {
      console.log("=== Stock In Edit: Data Loaded ===");
      console.log("Full Stock In data:", JSON.stringify(stockIn, null, 2));
      console.log("Stock In Products:", stockIn.stock_in_products);
      console.log("Stock In Products Length:", stockIn.stock_in_products?.length || 0);
      console.log("Available Products:", products?.length || 0);
      
      // Check if stock_in_products exists and has data
      if (!stockIn.stock_in_products || stockIn.stock_in_products.length === 0) {
        console.warn("⚠️ No stock_in_products found in data!");
        console.warn("⚠️ This is likely a backend issue - the API should include stock_in_products in the response.");
        console.warn("⚠️ Please check your backend to ensure the 'stock_in_products' relationship is loaded.");
        
        // If total_quantity > 0 but no products, show warning
        if (stockIn.total_quantity && stockIn.total_quantity > 0) {
          console.error("❌ ERROR: total_quantity is", stockIn.total_quantity, "but stock_in_products is empty!");
          console.error("❌ This indicates a backend bug - products exist but are not being returned.");
        }
        ...
        requestAnimationFrame(() => {
          replace([{ product_id: 0, quantity: 1, price: 0, expiry_date: "" }]);
        });
        return;
      }
      
      // Prepare products array with detailed logging
      const productsData = stockIn.stock_in_products.map((sip, idx) => {
        const productId = sip.product_id || sip.product?.id || 0;
        const quantity = sip.quantity || 1;
        const expiryDate = sip.expiry_date 
          ? (sip.expiry_date.includes("T") ? sip.expiry_date.split("T")[0] : sip.expiry_date) 
          : "";
        const price = sip.price || sip.batch_number || 0; // Support both price and batch_number for backward compatibility
      ...
      // Reset form with all data - use keepDefaultValues: false to ensure clean reset
      reset(formData, { keepDefaultValues: false });
      
      // IMPORTANT: Replace useFieldArray fields to ensure they update
      // Use requestAnimationFrame to ensure reset() completes and DOM is ready
      requestAnimationFrame(() => {
        replace(productsData);
        console.log("✅ Fields replaced with:", productsData.length, "products");
        console.log("✅ Current watched products:", watchedProducts);
      });
    }
  }, [stockIn, isPending, productsLoading, products, reset, replace]);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <Link to="/stock-management/stock-in" className="btn btn-primary font-semibold">
            Back to Stock In
          </Link>
        </div>
      </div>
            ...
            >
              ...
              <div className="text-center mb-3" style={{ padding: "0 2px" }}>
                ...
                <p className="text-xs text-gray-600 leading-snug">
                  Stock in data has been successfully updated.
                </p>
              </div>
      ...
      >
          <div className="flex items-center gap-6 h-[92px] bg-white w-full rounded-3xl p-[18px]">
            <div className="flex flex-col gap-[6px] w-full">
              <h1 className="font-bold text-2xl">Edit Stock In</h1>
              <p className="font-medium text-base text-monday-gray">
                Update stock in information
              </p>
              <Link
                ...
                <div className="flex flex-col gap-4">
                  <div>
                    ...
                    <p className="text-sm text-monday-gray">
                      Update products entering the warehouse
                    </p>
                  </div>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <label className="group relative">
                            ...
                            <input
                              ...
                              value={watchedProducts?.[index]?.expiry_date || ""}
                              className="appearance-none w-full h-[72px] font-semibold text-lg rounded-3xl border-[1.5px] border-monday-border pl-20 pr-6 pb-[14.5px] pt-[34.5px] placeholder-shown:pt-[14.5px] focus:border-monday-black transition-300"
                              placeholder=""
                            />
                          </label>
